...
def build_earnings_quality(fins: List[FinancialData], latest: Optional[FinancialData]) -> Dict[str, Any]:
    ...
    _SRC_LABELS = {'Xueqiu': '雪球', 'Yahoo Finance': 'Yahoo', 'Finnhub': 'Finnhub', 'SEC EDGAR': 'SEC', 'SEC Report': 'SEC解析', 'Manual Upload': '手动上传'}
...
def enrich_stock_for_display(stock: Stock, preloaded_fins: List[FinancialData] = None) -> Stock:
    ...
    # 数据来源标签
    _SOURCE_LABELS = {'Xueqiu': '雪球', 'Yahoo Finance': 'Yahoo', 'Finnhub': 'Finnhub', 'SEC EDGAR': 'SEC', 'SEC Report': 'SEC解析', 'Manual Upload': '手动上传'}
