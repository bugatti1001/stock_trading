"""
Unified AI Client
统一封装 Claude、OpenAI、MiniMax 和 NVIDIA 调用，所有 AI 服务通过此模块发起请求
"""
import logging
...
def _get_openai_client(api_key: str, base_url: Optional[str] = None):
    ...
    import openai
    cache_key = f"{api_key}|{base_url or 'openai'}"
...
def create_message(system: str = '',
    ...
    """
    调用 AI 生成回复，返回文本。
    自动选择 Claude、OpenAI、MiniMax 或 NVIDIA，也可显式指定 provider/api_key。
    """
    ...
    if prov == 'openai':
        return _openai_create(key, model, system, messages, max_tokens, temperature)
    elif prov == 'minimax':
        return _minimax_create(key, model, system, messages, max_tokens, temperature)
    elif prov == 'nvidia':
        return _nvidia_create(key, model, system, messages, max_tokens, temperature)
    else:
...
def stream_message(system: str = '',
    ...
    """
    流式调用 AI，逐段 yield 文本。
    自动选择 Claude、OpenAI、MiniMax 或 NVIDIA，也可显式指定。
    """
    prov, key, default_model = _get_provider_and_keys(provider, api_key)
    model = model or default_model
    messages = messages or []
    ...
    if prov == 'openai':
        yield from _openai_stream(key, model, system, messages, max_tokens)
    elif prov == 'minimax':
        yield from _minimax_stream(key, model, system, messages, max_tokens)
    elif prov == 'nvidia':
        yield from _nvidia_stream(key, model, system, messages, max_tokens)
    else:
...
def _openai_create(api_key, model, system, messages, max_tokens, temperature):
...
def _openai_stream(api_key, model, system, messages, max_tokens):
    client = _get_openai_client(api_key)
...
def _nvidia_create(api_key, model, system, messages, max_tokens, temperature):
    from app.config.settings import NVIDIA_BASE_URL
    client = _get_openai_client(api_key, NVIDIA_BASE_URL)
    kwargs = dict(
        model=model,
        messages=_build_openai_messages(system, messages),
        max_tokens=max_tokens,
...
def _nvidia_stream(api_key, model, system, messages, max_tokens):
    from app.config.settings import NVIDIA_BASE_URL
    client = _get_openai_client(api_key, NVIDIA_BASE_URL)
    stream = client.chat.completions.create(
        model=model,
