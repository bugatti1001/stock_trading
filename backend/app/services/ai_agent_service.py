...
def _build_system_prompt(user_message: str, context_mode: str, stock_id: Optional[int] = None) -> str:
...
def create_conversation(title: str = '新对话', context_mode: str = 'global',
                        stock_id: Optional[int] = None) -> Conversation:
    """新建对话"""
    mode: ContextMode = ContextMode.STOCK if context_mode == 'stock' else ContextMode.GLOBAL
    conv: Conversation = Conversation(title=title, context_mode=mode, stock_id=stock_id)
    db_session.add(conv)
    db_session.commit()
    return conv
